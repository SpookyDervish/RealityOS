...
void rw_sectors(const uint32_t size_in_sectors, uint32_t starting_sector, const uint32_t address, const uint8_t command)
{
    ...
    __asm__ __volatile__ ("outb %%al, %%dx" : : "a"(0xE0 | (starting_sector >> 24) & 0x0F), "d"(0x1F6) );  // Head/drive # port - send head/drive #
    __asm__ __volatile__ ("outb %%al, %%dx" : : "a"(size_in_sectors),                "d"(0x1F2) );  // Sector count port - # of sectors to read/write
    __asm__ __volatile__ ("outb %%al, %%dx" : : "a"(starting_sector & 0xFF),         "d"(0x1F3) );  // Sector number port / LBA low bits 0-7
    __asm__ __volatile__ ("outb %%al, %%dx" : : "a"((starting_sector >> 8)  & 0xFF), "d"(0x1F4) );  // Cylinder low port / LBA mid bits 8-15
    __asm__ __volatile__ ("outb %%al, %%dx" : : "a"((starting_sector >> 16) & 0xFF), "d"(0x1F5) );  // Cylinder high port / LBA high bits 16-23
