...
// Mainly used for keyboard input and nothing else lol
...
typedef struct
{
    uint16_t isr_address_low;   // Lower 16 bits if ISR address
...
// Interrut "frame" to pass to interrupt handlers / SIRs
typedef struct {
    uint32_t eip;
    uint32_t cs;
    uint32_t eflags;
...
{
    print_str("ERR: DEFAULT EXCEPTION HANDLER - NO ERROR CODE");
}

// Default exception handler (includes error code)
__attribute__ ((interrupt)) void default_excp_handler_err_code(int_frame_32_t *int_frame_32, unsigned long int error_code)
{
    print_str("DEFAULT EXCEPTION HANDLER - ERROR CODE: ");
    // TODO: actually print the code
}

// Default interrupt handler
__attribute__ ((interrupt)) void default_int_handler(int_frame_32_t *int_frame_32)
{
    print_str("DEFAULT INTERRUPT HANDLER");
}
